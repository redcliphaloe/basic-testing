...
describe('simpleCalculator tests', () => {
  ...
  test('should exponentiate two numbers', () => {
    expect(simpleCalculator({ a: 6, b: 7, action: Action.Exponentiate })).toBe(
      279936,
    );
  });

  test('should return null for invalid action', () => {
    expect(simpleCalculator({ a: 7, b: 8, action: 'x' })).toBeNull();
  });

  test('should return null for invalid arguments', () => {
    expect(simpleCalculator({ a: '8', b: '9', action: Action.Add })).toBeNull();
  });
});
